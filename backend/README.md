...
- **AWS RDS Oracle** への接続情報と **AWS SES** 用の認証情報（`dev` / `prod` いずれでも必須）
...
`.env` に `DB_URL` / `DB_USERNAME`（**`template_app`**）/ `DB_PASSWORD` / `AWS_*` / `JWT_SECRET` を設定します（`SPRING_PROFILES_ACTIVE` は通常 `dev`）。
