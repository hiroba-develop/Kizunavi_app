...
| シークレット名 | 用途 |
|---------------|------|
| `product-template/dev/db-credentials` | 開発環境の DB 接続情報 |
| `product-template/prod/db-credentials` | 本番環境の DB 接続情報 |
| `product-template/dev/app-secrets` | 開発環境のアプリ秘密情報 |
| `product-template/prod/app-secrets` | 本番環境のアプリ秘密情報 |
...
aws secretsmanager create-secret \
  --name "product-template/dev/db-credentials" \
  --description "Product Template 開発環境 DB 接続情報" \
  --secret-string '{
...
# 本番環境
aws secretsmanager create-secret \
  --name "product-template/prod/db-credentials" \
  --description "Product Template 本番環境 DB 接続情報" \
  --secret-string '{
...
# 開発環境
aws secretsmanager create-secret \
  --name "product-template/dev/app-secrets" \
  --description "Product Template 開発環境アプリ秘密情報" \
  --secret-string '{
...
# 本番環境
aws secretsmanager create-secret \
  --name "product-template/prod/app-secrets" \
  --description "Product Template 本番環境アプリ秘密情報" \
  --secret-string '{
...
6. **シークレットの名前**: `product-template/prod/db-credentials`
7. **説明**: `Product Template 本番環境 DB 接続情報`
8. **保存** をクリック
...
aws secretsmanager update-secret \
  --secret-id "product-template/prod/db-credentials" \
  --secret-string '{
...
aws secretsmanager list-secrets \
  --filter Key=name,Values=product-template \
  --region ap-northeast-1
```
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:secretsmanager:ap-northeast-1:123456789012:secret:product-template/prod/*"
      ]
    }
  ]
}
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "arn:aws:secretsmanager:ap-northeast-1:123456789012:secret:product-template/dev/*"
      ],
      "Condition": {
        "StringEquals": {
...
4. ポリシー名: `ProductTemplate-SecretsManager-ReadOnly`
5. **ポリシーを作成**
...
DB_CREDS=$(get_secret "product-template/${ENV}/db-credentials")
APP_SECRETS=$(get_secret "product-template/${ENV}/app-secrets")
...
```ini
[Unit]
Description=Product Template (docker compose)
...
java -jar /opt/app/product-template.jar
```
...
spring:
  config:
    import: optional:aws-secretsmanager:product-template/${spring.profiles.active}/db-credentials;product-template/${spring.profiles.active}/app-secrets
...
| **AWS dev EC2** | Secrets Manager + 起動スクリプト | `product-template/dev/*` 等 |
| **AWS prod EC2** | Secrets Manager + 起動スクリプト | `product-template/prod/*` 等 |
...
aws secretsmanager get-secret-value \
  --secret-id "product-template/dev/db-credentials" \
  --region ap-northeast-1 \
  --query 'SecretString' \
  --output text | jq -r 'to_entries[] | "\(.key)=\(.value)"' > .env

aws secretsmanager get-secret-value \
  --secret-id "product-template/dev/app-secrets" \
  --region ap-northeast-1 \
  --query 'SecretString' \
  --output text | jq -r 'to_entries[] | "\(.key)=\(.value)"' >> .env
...
6. Lambda 関数名: `product-template-secret-rotation`
...
- シークレット名のプレフィックス（`product-template/prod/`）が一致しているか確認
- リージョンが一致しているか確認
...
- [ ] シークレットの命名規則を決定（`product-template/{env}/{type}`）
